...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def forward(
        ...
        if video_pixel_values is not None:
            #print('video_pixel_values', video_pixel_values.dtype)
        ...
        for b in range(batch_size):
            start = 0
            result = []
            print('media token indices len', len(media_token_indices[b]))
            if len(media_token_indices[b]) > 1: #ssaha3 changed this from 0 to 1
        ...
        print('Loss mask shape1', loss_mask.shape)
        ...
        print(f"Loss mask shape2: {loss_mask.shape}")
